        ...
        <div className="mx-auto w-full max-w-6xl">
          <header className="max-w-3xl">
            <p className="mb-3 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-amber-deep dark:text-amber">
              <span
                className="h-px w-6 bg-amber-deep/60 dark:bg-amber/60"
                aria-hidden="true"
              />
              Support
            </p>
            ...
            <p className="mt-6 max-w-2xl text-[17px] leading-relaxed text-ink/75 dark:text-cream/75">
              We read every message. Send us the details and we will help you
              get back to logging smarter trips.
            </p>
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {supportAreas.map(({ title, description, icon: Icon }) => (
              <article
                key={title}
                className="rounded-2xl border border-ink/10 bg-card p-6   dark:border-cream/10 dark:bg-carddark"
              >
                ...
                <h2 className="mt-4 font-display text-2xl text-ink dark:text-cream">
                  {title}
                </h2>
                <p className="mt-3 text-sm leading-relaxed text-ink/75 dark:text-cream/75">
                  {description}
                </p>
              </article>
            ))}
          ...
          <div className="mt-12 rounded-2xl border border-ink/10 bg-paper-deep/70 p-8 dark:border-cream/10 dark:bg-herodark/60">
            <h2 className="font-display text-3xl text-ink dark:text-cream">
              Contact support
            </h2>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-ink/75 dark:text-cream/75">
              Email us anytime and include as much context as possible so we can
              help faster.
            </p>
