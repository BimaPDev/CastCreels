    ...
    <Section
      ...
      title={
        <>
          ...
          <em className="italic text-amber-deep dark:text-amber">fish</em>.
        </>
      }
    ...
    >
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {features.map(({ icon: Icon, title, body }) => (
          <article
            key={title}
            className="group relative rounded-xl border border-ink/10 bg-card p-6   transition-shadow duration-200 hover:shadow-raised dark:border-cream/10 dark:bg-carddark"
          >
